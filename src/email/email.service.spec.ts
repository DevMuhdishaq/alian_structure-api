...
import { EmailLog, EmailStatus, EmailProvider } from "./entities/email-log.entity";
...
describe("EmailService", () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [EmailService, TemplateEngineService,
        { provide: getRepositoryToken(EmailLog), useFactory: mockEmailLogRepository },
  ...
  describe("sendEmail", () => {
    it("should queue a single email", async () => {
      const dto: SendEmailDto = { to: ["user@example.com"], subject: "Test", html: "<p>Hello</p>", text: "Hello" };
    ...
    it("should send to multiple recipients", async () => {
      const dto: SendEmailDto = { to: ["a@x.com", "b@x.com"], subject: "Multi", html: "<p>Bulk</p>" };
    ...
    it("should render template when templateName provided", async () => {
      const dto: SendEmailDto = { to: ["user@example.com"], subject: "Welcome", templateName: "welcome", templateVars: { name: "Alice" } };
  ...
  describe("sendBulk", () => {
    it("should queue multiple emails", async () => {
      ...
      queueService.enqueueEmail.mockResolvedValue({ id: 1 } as any);
      const r = await service.sendBulk({ emails: [{ to: ["a@x.com"], subject: "A", html: "<p>A</p>" }, { to: ["b@x.com"], subject: "B", html: "<p>B</p>" }] } as any);
  ...
  describe("getDeliveryStatus", () => {
    it("should return email log by id", async () => {
      emailLogRepo.findOne.mockResolvedValue({ id: "u1", status: EmailStatus.SENT } as any);
      expect((await service.getDeliveryStatus("u1")).id).toBe("u1");
    });
    it("should throw when not found", async () => {
      emailLogRepo.findOne.mockResolvedValue(null);
      await expect(service.getDeliveryStatus("missing")).rejects.toThrow("not found");
    });
  });

  describe("retryFailed", () => {
    it("should requeue a failed email", async () => {
      emailLogRepo.findOne.mockResolvedValue({ id: "f1", status: EmailStatus.FAILED } as any);
      queueService.enqueueEmail.mockResolvedValue({ id: 1 } as any);
      await service.retryFailed("f1");
      expect(emailLogRepo.update).toHaveBeenCalledWith("f1", { status: EmailStatus.QUEUED, attempts: 0, errorMessage: null });
    });
    it("should throw if not failed", async () => {
      emailLogRepo.findOne.mockResolvedValue({ id: "s1", status: EmailStatus.SENT } as any);
      await expect(service.retryFailed("s1")).rejects.toThrow("not in failed status");
    });
  });
  ...
  describe("unsubscribe", () => {
    it("should mark recipient as unsubscribed", async () => {
      ...
      expect((await service.unsubscribe("user@example.com")).unsubscribed).toBe(true);
    });
  });

  describe("template rendering", () => {
    it("should render template with variables", () => { expect(templateEngine.renderHtml("welcome", { name: "Bob" })).toContain("Bob"); });
    it("should HTML-escape variable values", () => { expect(templateEngine.renderHtml("welcome", { name: "<script>" })).not.toContain("<script>"); });
    it("should list all default templates", () => { expect(templateEngine.getAllTemplates().length).toBeGreaterThanOrEqual(4); });
  });

  describe("createTemplate", () => {
    it("should create and store a template", async () => {
      const r = await service.createTemplate({ name: "custom", htmlContent: "<p>Hey {{name}}</p>", subject: "Hi {{name}}" } as any);
      expect(r.name).toBe("custom");
      expect(templateEngine.renderHtml("custom", { name: "Carol" })).toContain("Carol");
    });
  });
});