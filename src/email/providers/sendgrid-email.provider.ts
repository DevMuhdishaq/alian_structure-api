...
export class SendgridEmailProvider implements EmailProvider {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    if (this.apiKey) { this.logger.log("SendGrid provider initialised"); }
    else { this.logger.warn("SendGrid provider initialised without API key -- unavailable"); }
  }

  async sendEmail(options: SendEmailOptions): Promise<SendEmailResult> {
    if (!this.apiKey) { throw new Error("SendGrid API key is not configured"); }
