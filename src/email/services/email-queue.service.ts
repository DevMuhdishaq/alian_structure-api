...
import { EmailProvider as IEmailProvider, SendEmailOptions } from "../interfaces/email-provider.interface";
import { SmtpEmailProvider } from "../providers/smtp-email.provider";
import { SendgridEmailProvider } from "../providers/sendgrid-email.provider";
import { SesEmailProvider } from "../providers/ses-email.provider";

export interface EmailJobData { emailLogId: string; }
...
export class EmailQueueService {
  ...
  async enqueueEmail(emailLogId: string): Promise<Job<EmailJobData>> {
    const job = await this.emailQueue.add("send-email", { emailLogId }, { attempts: 5, backoff: { type: "exponential", delay: 2000 }, removeOnComplete: 100, removeOnFail: 500 });
  ...
  async processEmail(job: Job<EmailJobData>): Promise<void> {
    ...
    if (!emailLog) { throw new Error(`EmailLog ${emailLogId} not found`); }
    if (emailLog.unsubscribed) { await this.updateStatus(emailLogId, EmailStatus.FAILED, "Recipient unsubscribed"); return; }
    if (emailLog.status === EmailStatus.DELIVERED) { return; }
    await this.updateStatus(emailLogId, EmailStatus.SENDING);
    const provider = this.providers.get(emailLog.provider);
    if (!provider) { throw new Error(`Provider "${emailLog.provider}" not found`); }
    if (!(await provider.isAvailable())) { throw new Error(`Provider "${emailLog.provider}" is not available`); }
  ...
  async getPendingEmailCount(): Promise<number> { return this.emailQueue.count(); }
  async getFailedEmailLogs(): Promise<EmailLog[]> { return this.emailLogRepository.find({ where: { status: EmailStatus.FAILED }, order: { createdAt: "DESC" } }); }

  private async updateStatus(id: string, status: EmailStatus, errorMessage?: string): Promise<void> {
